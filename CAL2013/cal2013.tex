...
Levenshtein distance to measure the difference of large program
traces. This algorithm calculates the similarity of two traces and
reconstructs edit sequences, so that engineers know how similar
the two traces are and where the difference is. We present a joint
trace grouping, filtering and partition technology that
significantly reduces the complexity of Levenshtein
distance computation. We apply our algorithm to real program traces collected by
a Pin-tool and the full system simulator GEM5. Experimental results
show that our traces comparison algorithm is precise, efficient and
scalable.
\end{abstract}
...
limitations including complexity, inaccuracy and short length. %For
%example, Tracediff~\cite{tracediff} is a useful tool provided by the
%GEM5 simulator. It simply compares the two traces instruction by
%instruction and skips instructions to find a synchronization point.
%In a full system environment, program traces may changes from
%execution to execution due to thread scheduling and communication.
...
%In instruction trace comparison, we concern not only the type of
%instructions, but also the number of the same instructions appeared
%in both traces, but also the sequential order that instructions are
%generated and appear in the traces.
...
%\hline GEM5-x86-linux & X86/2.0 GHz/4-core & &  Linux 2.6.22.9.smp & gcc 4.5.2 \\
...
%All of the benchmarks are selected from SPLASH2~\cite{splash295} and they are compiled using a
%same compiler and each binary is executed in both the host machine
%and the simulated full system in GEM5.
...
\bibitem{CCPP}
Quantifying Behavioral Difference Between C and C++ programs
