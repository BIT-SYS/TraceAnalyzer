...
the behavior of applications. However, large program traces, each of
which is composed of millions of instructions, are difficult to
analyze and understand. In this paper, we present an efficient
algorithm based-on Levenshtein distance to measure the differences
of large program traces. This algorithm calculates the similarity
between traces and reconstructs the edit sequences, so that we know
how similar the two traces are and where the difference is. We apply
our algorithm to real program traces collected by a pin-tool.
Experimental results show that our traces comparison algorithm is
...
widely used for program optimization, program debugging and new
architecture evaluation. The comparison of program traces reveals
important information about the behavior of two executions. However,
low level program traces, such as instruction sequences, tend to be
considerably large and usually contain thousands of millions
instructions, which in turn hinders their effective analysis. Some
existing trace analysis tools work on high level program traces,
such as control flow graph, call graph and internal event sequences,
and rely on visualization techniques to help software engineers make
sense of trace
...
Currently, we didn't find any algorithms reported in the literature
to measure the differences of very large instruction traces
efficiently and effectively. Most trace analyzing tools suffer from
a variety of limitations including complexity, inaccuracy and short
length. In this paper, we propose a new algorithm to measure the
differences of two very large traces based-on Levenshtein distance.
This algorithm is precise and scalable to find out how similar the
two traces are and where the difference is.
...
similarity relations between strings~\cite{Faulk1964}. In the
process of traces comparison, we concern not only the types of the
instructions and the number of the same instructions appeared in
both traces, but also the sequential order that instructions are
generated and arranged in the traces. Although the same executable
binary is used to collect traces in different runs, we expect that
the traces will be slightly different to each other due to thread
scheduling and other interferences from the system. As it turned
out, we found these differences in the collected traces.
...
the minimum number of edits needed to transform the first string
into the second, with the allowable edit operations being insertion,
...
between "trcet" and "traces" is 2, since an insertion of "a" after
"r" and a substitution of "t" for "s" at the end are required to
change "trcet" to "traces".
...
\begin{equation}\label{eq:sim}
 S_{t_{1},t_{2}} = \frac{(\max{(|t_1|,|t_{2}|)} - \sum_{i=1}^{n}lev_{t_{1,i}, t_{2,i}}(|t_{1,i}|,|t_{2,i}|))}{
 \max{(|t_1|,|t_{2}|)}}
\end{equation}

where $n$ is the number of threads. If $t_{1,i}$ and $t_{2,i}$ are
the same, then $lev_{t_{1,i}, t_{2,i}}=0$. Thereby, the larger the
$S_{t_{1},t_{2}}$ is, the more similar the two traces are.
...
\subsubsection{Similarity Calculation Based-on Snippets}
The computation complexity of current Levenshtein distance algorithm
depends on the lengths of the two strings. If both strings $a$ and
$b$ can be split into a number of substrings and Levenshtein
distances are calculated for each pair, then both time and space
complexity could be reduced. In general, this is difficult for
...
some of the instructions have special meanings in the sequences. For
example, the "call" and "return" instructions appear in pair in the
traces, and the pairs are usually structurally and hierarchically
nested. If the two traces are the same, we are certainly sure that
the two threads have the same function invocation paths, on which
instructions are executed with "call" and "return"s. Therefore, if
we can rebuild the dynamic call paths from the two traces, we can
firstly compare the two traces in a higher abstract level to check
if the two threads diverge at some points on their call paths. In
order to do that, we pick out the "call" and "return" instructions
from the two traces and generate another two smaller traces, which
are called "call traces" of the two threads. Then, we compute the
Levenshtein distance for these two call traces and reconstruct the
edit sequence. After that, we know a number of synchronization
...
The extracted traces for each thread from different systems contain
both PC addresses and instructions executed. Algorithm
~\ref{alg:similarity} shows how the similarity $s$ of two traces
$t_1$ and $t_2$ is calculated. At first, two call traces $c_1$ and
$c_2$ are generated from $t_1$ and $t_2$ respectively. Then we
compute the edit distance for these two call traces and reconstruct
the edit distance $e$. Now $e$ contains all the operation of
deletion, insertion and substitute that are performed on symbols in
$c_1$ to transform $c_1$ into $c_2$. We remove all the "call"s and
"return"s that are involved in $e$ from $c_1$. The new call traces
$c'_1$ generated from $c_1$ now only includes "call"s and "return"s
that appear in both $t_1$ and $t_2$ with positional similarity and
ordinal similarity. After that, we split $t_1$ and $t_2$ into a
number of subsequences according to the "call"s and "return"s
included in $c'_1$. We compare each pair of the subsequences and
calculate the final similarity of the two traces using equation
~\ref{eq:sim}.
...
Reconstruction the edit sequence $e$ for $c_1$ and $c_2$\;

$c'_1 = c_1 - e$\;

Split $t_1$ into a collection of subsequences $C=\{t_{1i}|1\leq i
\leq |c'_1|\}$\;

Split $t_2$ into a collection of subsequences $C=\{t_{2i}|1\leq i
\leq |c'_1|\}$ \;

$ed = 0$\;

\For{$i=1$ to $|c'_1|$}{
    $ed = ed + Lev_{}(|t_{1i}|,|t_{2i}|)$\;
}

$s={(\max{(|t_1|,|t_{2}|)} - ed)}/{\max{(|t_1|,|t_{2}|)}}$\;
